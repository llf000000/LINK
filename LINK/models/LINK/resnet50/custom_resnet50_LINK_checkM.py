...
class SPP(nn.Module):
    def __init__(self, M=None):
        ...
        self.pooling_3x3 = nn.AdaptiveAvgPool2d((3, 3))
    ...
    def forward(self, x):
        ...
        x_3x3_flatten = torch.flatten(x_3x3, start_dim=2, end_dim=3)

        if self.M == '[1]':
            x_feature = torch.cat((x_1x1_flatten,), dim=2)
        elif self.M == '[2]':
            x_feature = torch.cat((x_2x2_flatten,), dim=2)
        elif self.M == '[3]':
            x_feature = torch.cat((x_3x3_flatten,), dim=2)
        elif self.M == '[4]':
            x_feature = torch.cat((x_4x4_flatten,), dim=2)
        elif self.M == '[1,2]':
            x_feature = torch.cat((x_1x1_flatten, x_2x2_flatten), dim=2)
        elif self.M == '[1,3]':
            x_feature = torch.cat((x_1x1_flatten, x_3x3_flatten), dim=2)
        elif self.M == '[1,4]':
            x_feature = torch.cat((x_1x1_flatten, x_4x4_flatten), dim=2)
        elif self.M == '[2,3]':
            x_feature = torch.cat((x_2x2_flatten, x_3x3_flatten), dim=2)
        elif self.M == '[2,4]':
            x_feature = torch.cat((x_2x2_flatten, x_4x4_flatten), dim=2)
        elif self.M == '[1,2,4]':
            x_feature = torch.cat((x_1x1_flatten, x_2x2_flatten, x_4x4_flatten), dim=2)
        elif self.M == '[1,2,3]':
            x_feature = torch.cat((x_1x1_flatten, x_2x2_flatten, x_3x3_flatten), dim=2)
        elif self.M == '[1,3,4]':
            x_feature = torch.cat((x_1x1_flatten, x_3x3_flatten, x_4x4_flatten), dim=2)
        elif self.M == '[1,2,3,4]':
            x_feature = torch.cat((x_1x1_flatten, x_2x2_flatten, x_3x3_flatten, x_4x4_flatten), dim=2)
        else:
